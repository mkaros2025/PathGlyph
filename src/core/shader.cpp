...
Shader::Shader(const std::string& vertexPath, const std::string& fragmentPath) {
    ...
    if (!fs::exists(vertexPath)) {
        std::cerr << "错误：顶点着色器文件不存在: " << fs::absolute(vertexPath).string() << std::endl;
        throw std::runtime_error("顶点着色器文件不存在");
    }
    
    if (!fs::exists(fragmentPath)) {
        std::cerr << "错误：片段着色器文件不存在: " << fs::absolute(fragmentPath).string() << std::endl;
    ...
    try {
        ...
        std::cout << "着色器文件读取成功" << std::endl;
        std::cout << "  顶点着色器: " << fs::absolute(vertexPath).string() << " (" << vertexCode.size() << " 字节)" << std::endl;
        std::cout << "  片段着色器: " << fs::absolute(fragmentPath).string() << " (" << fragmentCode.size() << " 字节)" << std::endl;
