...
class Renderer {
 public:
  ...
  // Renderer 类管理了大量的资源，包括指针、OpenGL 资源（如
  // VAO、VBO）、着色器等。 这些资源通常不能被简单地拷贝
  // 否则会出资源冲突之类乱七八糟的问题
 ...
 private:
  ...
  GLuint createVAO();
  // 创建一个 顶点缓冲对象（VBO, Vertex Buffer Object），并将顶点数据上传到
  // GPU。
  GLuint createVBO(const void* data, size_t size, bool dynamic = false);
  // 创建一个 索引缓冲对象（EBO, Element Buffer Object），并将索引数据上传到
  // GPU。
  GLuint createEBO(const void* data, size_t size);
  // 更新一个已存在的 顶点缓冲对象（VBO） 的数据。
  void updateVBO(unsigned int vbo, const void* data, size_t size,
                 size_t offset = 0);
  ...
  // 更新代理几何数据
  // 功能：根据迷宫中的代理（如起点、终点、当前位置）信息，生成或更新代理的顶点数据并上传到
  // GPU。 主要用于渲染迷宫中的特殊点（如起点、终点）。
